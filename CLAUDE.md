...
- **/homeworkout page LIVE (deployed July 2, 2026)** (`homeworkout/index.html`): free home-workout landing page (warmup + 5-move circuit: squat, single-leg RDL with a gallon of water, push-up, backpack row, **bird dog** [chosen over plank; McGill Big 3]). Demo photos still pending (hero + squat/rdl/pushup/row/birddog → drop into `homeworkout/images/` and redeploy); the `onerror` handlers hide missing images so the page reads fine text-only. Column #2 and the desk sign point here.
- **Site nav menu + /articles section LIVE (July 2, 2026).** Homepage nav now has Home workout + Articles links, with a plain hamburger dropdown on mobile (≤760px; small inline JS toggle at the bottom of each page). `articles/index.html` = listing page; `articles/lose-the-fat-keep-the-muscle/index.html` = full text of Wave column #1, verbatim from the version Mark approved by email June 27, 2026 (photo = `photos/jamal-inhome-session.jpg`). When The Wave publishes it online, add the "Read it in The Wave" link (HTML comment placeholder is in the article page below the meta line). Each future column gets its own `articles/<slug>/index.html` + a card on the listing page.
